drop table cuisinetype cascade constraints;
...
drop sequence cuisinetype_seq;
drop sequence customerid_seq;
drop sequence muid_seq;
...
exec insertcustomerID('CustPA5','CustPA5@umbc.edu','Goa Street','Philedelphia','PA',16822,9876545678654324);
...
resaddr varchar(30);
begin
dbms_output.put_line('-------------------------- Show restaurant by cuisine name BY MEMBER 1 ---------------');
open c1;
loop
...
exec return_restaurant_info('Ethiopian');

-----------------REPORT BY MEMBER 1 (Maulik Soni)----------------------------------
...
begin
dbms_output.put_line('The inital inventory for Restaurant ' || res_name ||' is: ');
for r in c1 loop
...
set serveroutput on;
exec show_restaurant_inventory('Selasie');
exec show_restaurant_inventory('Taj Mahal');
exec show_restaurant_inventory('Bull Roast');
...
if res_name ='Ethiop'then
dbms_output.put_line('---------------  Initial Inventory for Ethiop restaurant -------------------');
show_restaurant_inventory(res_name);
else
dbms_output.put_line('The original inventory for '||menu_name||' in restaurant '|| res_name ||' is: '||qty);
end if;
...
dbms_output.put_line('The updated inventory for '||menu_name||' in restaurant ' || res_name ||' is: '||qty);
end if;
else
...
as
cursor c1 is select cuisine_name,menu_item_name,price from menu_items m, cuisinetype c where m.cuisine_type_id=c.cuisine_type_id;
begin
...
dbms_output.put_line('Menu Item is: '||r.menu_item_name|| ',Cuisine name is :'||r.cuisine_name||',and Price: '||r.price);
end loop;
end;
/
...
is
cursor c1 is 
SELECT * FROM (select c.name from customer c 
join orders o on o.customer_ID=c.customer_ID group by c.name order by max(o.amount_paid) desc) where rownum<=3 ;

cursor c2 is 
SELECT * FROM (select c.name from customer c 
join orders o on o.customer_ID=c.customer_ID group by c.name order by max(o.amount_paid) asc) where rownum<=3 ;
...
loop
 fetch c2 into cs_name;
    exit when c2%notfound;
    
    dbms_output.put_line(cs_name || ' ');
end loop;
